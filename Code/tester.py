...
class Inference():
    ...
    def evaluate_metrics(self, image_id, pred_tags_prob, pred_impression, pred_findings):
        ...
        '''
        gt_tags, gt_impression, gt_findings = self.get_true_data(image_id)
        active_tags = np.count_nonzero(gt_tags == 1)
        hamming = metrics.hamming(tags, pred_tags_prob, active_tags)
        impressions_similarity = metrics.semantic_similarity(pred_impression, gt_impression)
        findings_similarity = metrics.semantic_similarity(pred_findings, gt_findings)
